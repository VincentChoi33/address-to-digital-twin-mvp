...
export async function validateNvidiaPackage(packageDir: string): Promise<NvidiaPackageValidationReport> {
  ...
  checks.push(passIf("WARP.FLOOD_SMOKE.001", ["import warp as wp", "@wp.kernel", "wp.launch", "shallow_water_step", "flooded_area_gt_10cm", "--allow-missing"].every((token) => warpFloodScript.includes(token)), "NVIDIA Warp flood smoke script contains a CUDA Warp kernel, launch path, 10cm flood acceptance, and blocked-mode audit."));
