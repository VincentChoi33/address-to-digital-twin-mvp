  ...
  return {
    ...
    nvidia_only_target_stack: [
      ...
      },
      {
        ...
        role: "GPU-host shallow-water flood simulation smoke replacing browser MVP water as NVIDIA-only hydrology runtime evidence.",
        status: runtimeProbe.nvidiaSmi === "available" ? "runtime_check_required" : "blocked_missing_local_nvidia_runtime"
      },
      {
...
function buildSimReadyMinimumReport(
  ...
  return {
    ...
    next_runtime_commands: [
      ...
      "Run npm run nvidia:content-agents:deploy:plan, then run npm run nvidia:content-agents:deploy -- up on a GPU host with NVIDIA_API_KEY if Material/Physics endpoints are not already running; use deploy:status -- --wait-seconds 900 and source .tmp/nvidia-content-agents/endpoints.env before assignment.",
...
python3 nvidia_warp_flood_smoke.py --stage ${twin.project_id}.usda --output-json warp_flood_report.json --output-pgm warp_flood_depth.pgm
