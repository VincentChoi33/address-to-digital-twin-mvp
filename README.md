...
npm run nvidia:acceptance:check      # single NVIDIA-only acceptance certificate; blocked until Content Agents actually run
npm run nvidia:finish:check          # one-shot final chain; blocked safely until endpoints/key exist
npm run nvidia:finish:remote:check -- --host train1  # SSH GPU-host readiness probe; no secrets printed
npm run nvidia:key:register-and-finish -- --host train1  # hidden-prompt key install, then remote finish
...
| `npm run nvidia:acceptance` | aggregate the NVIDIA-only acceptance gates and fail until OpenUSD, ovrtx, ovstream, Warp, SimReady, viewer contract, and Content Agents all pass |
| `npm run nvidia:acceptance:check` | same acceptance report, but returns 0 for the current explicit blocked state so CI can prove the exact remaining gate |
| `npm run nvidia:finish` | one-shot finalizer: deploy/wait Content Agents when `NVIDIA_API_KEY` exists, run Material→Physics assignment, rerun SimReady, then rerun NVIDIA-only acceptance |
| `npm run nvidia:finish:check` | same finalizer in safe blocked mode for CI/local audits; it never calls dead generated endpoints before health is ready |
| `npm run nvidia:finish:remote` | SSH wrapper that updates/clones the repo on an NVIDIA GPU host, runs `nvidia:finish`, and copies finish evidence back locally |
| `npm run nvidia:finish:remote:check` | remote dry-run/preflight mode for `train1`-style GPU hosts; reports GPU/Docker/key/endpoint readiness without printing secrets |
| `npm run nvidia:key:register` | hidden-prompt helper that installs `NVIDIA_API_KEY_FILE` as `0600` on the remote GPU host without printing the key |
| `npm run nvidia:key:register-and-finish` | installs the key on the GPU host, then immediately runs `nvidia:finish:remote` |
...
The USD stage is authored as meter-based, Y-up OpenUSD with `MaterialBindingAPI`, `UsdPreviewSurface` materials, a USD `PhysicsScene`, conservative `PhysicsCollisionAPI` static colliders for terrain/buildings/roads/parcel geometry, `PhysicsMassAPI` building rigid bodies, official building meshes, road ribbons, parcel boundary, terrain reference, and flood-water reference layer. The ovrtx wrapper sublayers that source stage and adds NVIDIA viewer-owned Camera → RenderProduct → RenderVar → RenderSettings wiring. Remote `train1` evidence under [`docs/evidence/`](docs/evidence/) proves GPU/Docker/NVIDIA Container Toolkit/OpenUSD Python/Python ovrtx/Python ovstream/package-validation gates, a real 1280×720 ovrtx LdrColor first frame, ovrtx→ovstream server readiness (`/healthz` 503→200 after BGRA CUDA conversion), an actual browser `@nvidia/ov-web-rtc` Direct video first frame (`videoWidth=1280`, `videoHeight=720`, `readyState=4`), a passed NVIDIA Warp/CUDA shallow-water smoke (`max_depth_m=0.1577`, `flooded_area_m2_gt_10cm=147019.7`), and a read-only Content Agents deployment audit plus concrete deploy-plan showing GPU/Docker/Compose/NVIDIA Container Toolkit/upstream checkout/ports are ready on `train1`; the official Material/Physics stacks are remapped to `8100/8200` for simultaneous same-host deployment. [`docs/evidence/nvidia-only-acceptance-sadang-2026-06-13.md`](docs/evidence/nvidia-only-acceptance-sadang-2026-06-13.md) is the single acceptance certificate, and [`docs/evidence/nvidia-finish-sadang-2026-06-13.md`](docs/evidence/nvidia-finish-sadang-2026-06-13.md) is the one-shot finalizer report. OpenUSD, no-WebGL NVIDIA viewer contract, ovrtx, ovstream, Warp, and SimReady are passed; Content Agents deploy/runtime remain blocked. Remaining NVIDIA-only gate: provide healthy Content Agents Material/Physics endpoints, or provide `NVIDIA_API_KEY` and run `npm run nvidia:finish` on the GPU host to deploy missing Content Agents services, run material/physics assignment, rerun SimReady, and rerun acceptance. From the local Mac, `npm run nvidia:key:register-and-finish -- --host train1` prompts for the key without echoing it, stores it as `~/.secrets/nvidia_api_key` on `train1`, performs the same finish chain over SSH, builds Content Agents with host-network DNS for apt/pip, and copies the evidence back.
