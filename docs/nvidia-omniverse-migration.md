...
| Omniverse Content Agents | Material/physics assignment | Repo command and read-only deployment audit authored; train1 has GPU/Docker/Compose/NVIDIA Container Toolkit/upstream/ports ready, but `nvidia:content-agents:check` stays blocked until healthy Material/Physics service endpoints exist or `NVIDIA_API_KEY` deployment auth is provided for the upstream deployment handoff |
...
5. Provide healthy Content Agents Material/Physics endpoints, or provide `NVIDIA_API_KEY` to deploy the missing OVRTX/Material/Physics services from the NVIDIA upstream `content-agents` checkout, then run `npm run nvidia:content-agents` for material and physics assignment.
6. Re-run `npm run nvidia:simready` and persist the reports after Content Agents output is produced.
