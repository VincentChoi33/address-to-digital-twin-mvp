...
5. Provide healthy Content Agents Material/Physics endpoints, or set `NVIDIA_API_KEY` on the GPU host and run `npm run nvidia:content-agents:deploy -- up` to start the remapped NVIDIA upstream Material/Physics stacks (`8100/8200`), then run `npm run nvidia:content-agents` for material and physics assignment.
6. Re-run `npm run nvidia:simready` and `npm run nvidia:acceptance` after Content Agents output is produced.
