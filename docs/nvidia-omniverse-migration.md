...
| Omniverse Content Agents | Material/physics assignment | Repo command authored; `nvidia:content-agents:check` records blocked status until healthy Material/Physics service endpoints exist or `NVIDIA_API_KEY` deployment auth is provided for the upstream deployment handoff |
