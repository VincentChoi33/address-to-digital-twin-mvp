...
- Generated: `2026-06-15T03:08:31.281443Z`
...
| `SIMREADY.VALIDATOR.001` | `passed` | NVIDIA SimReady validator passed the self-contained asset-source copy | status=passed, profile=Prop-Robotics-Neutral@1.0.0, validator=available |
| `CONTENT_AGENTS.DEPLOY_READY.001` | `passed` | Official NVIDIA Content Agents deployment path is ready and/or already healthy | status=ready, blockers=[], gpu_assignment={'policy': 'auto_multi_gpu_split', 'detected_gpu_count': 8, 'material_ovrtx_visible_devices': '0', 'physics_ovrtx_visible_devices': '1'}, endpoint_env=.tmp/nvidia-content-agents/endpoints.env, source=docs/evidence/nvidia-content-agents-deploy-status-sadang-2026-06-13.json |
| `CONTENT_AGENTS.RUNTIME.001` | `passed` | NVIDIA Content Agents Material→Physics assignment has actually run | status=passed, blockers=[] |
